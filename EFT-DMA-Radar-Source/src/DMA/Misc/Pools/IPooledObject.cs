...
namespace LoneEftDmaRadar.Common.Misc.Pools
{
    /// <summary>
        ...
        where T : class, IPooledObject<T>
    {
        ...
        static void Return(T obj)
        {
            ...
            {
                DebugLogger.LogDebug($"CRITICAL ERROR: Unable to return '{obj.GetType()}' object to the ObjectPool!");
            }
        }
        ...
        private static class ObjectPool
        {
            ...
            internal static T Rent()
            {
                ...
                {
                    //DebugLogger.LogDebug($"{DateTime.UtcNow.Ticks}: New Pool Obj {typeof(T)}");
